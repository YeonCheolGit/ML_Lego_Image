...
W = tf.Variable(tf.random_normal([2500, nb_classes]))
...
# Test model
is_correct = tf.cast(hypothesis > 0.5, dtype=tf.float32)
# Calculate accuracy
accuracy = tf.reduce_mean(tf.cast(tf.equal(is_correct, Y), dtype=tf.float32))
