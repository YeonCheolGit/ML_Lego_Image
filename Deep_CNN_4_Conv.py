...
L3_flat = tf.reshape(L3, [-1, 128 * 7 * 7])

W4 = tf.get_variable("W4", shape=[128 * 7 * 7, 2500], initializer=tf.contrib.layers.xavier_initializer()) ## 마지막 weight층 출력 개수 증가
b4 = tf.Variable(tf.random_normal([2500]))
L4 = tf.nn.relu(tf.matmul(L3_flat, W4) + b4)
L4 = tf.nn.dropout(L4, keep_prob=keep_prob)

W5 = tf.get_variable("W5", shape=[2500, nb_classes], initializer=tf.contrib.layers.xavier_initializer())
